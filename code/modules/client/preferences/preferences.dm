...
/*
		if(href_list["task"] == "med_record") //med_record_here
...
/datum/preferences/proc/process_link(mob/user, list/href_list)
	...
	else if(href_list["preference"] == "set_med_record")
		switch(href_list["task"])
			if("open")
				SetMedicalRecord(user)
				return
			if("done")
				user << browse(null, "window=set_med_record")
				ShowChoices(user)
				return
			if("Default")
				var/msg = input(usr,"Set the default flavour text for your robot. It will be used for any module without individual setting.","Flavour Text",html_decode(flavour_texts_robot["Default"])) as message
